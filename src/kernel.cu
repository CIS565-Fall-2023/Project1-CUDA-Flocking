...
__global__ void kernComputeIndices(int N, int gridResolution,
  ...
  /*int x = (int)(diff.x * inverseCellWidth);
  int y = (int)(diff.y * inverseCellWidth);
  int z = (int)(diff.z * inverseCellWidth);*/
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  glm::vec3 cellIdx = (pos[iSelf] - gridMin) * inverseCellWidth;
  // - Identify which cells may contain neighbors. This isn't always 8.
#if TEST2_2
  glm::ivec3 cellStartIdx = cellIdx - 1.f;
  glm::ivec3 cellEndIdx = cellIdx + 1.f;
#else
  glm::ivec3 cellStartIdx = cellIdx - glm::vec3(0.5f); //TODO: bug fix!
  glm::ivec3 cellEndIdx = cellStartIdx + glm::ivec3(1); //TODO: bug fix!
#endif
    ...
    for (int y = cellStartIdx.y; y <= cellEndIdx.y; y++) {
      ...
      for (int x = cellStartIdx.x; x <= cellEndIdx.x; x++) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
          ...
          if (indexReal == iSelf) continue;
          float dist = glm::distance(pos[indexReal], pos[iSelf]);
          if (dist < rule1Distance) {
            perceivedCenter += pos[indexReal];
            rule1Neighbors++;
          }
          if (dist < rule2Distance) {
            c -= (pos[indexReal] - pos[iSelf]);
          }
          if (dist < rule3Distance) {
  ...
  glm::vec3 newVel = vel1[iSelf] + (perceivedCenter - pos[iSelf]) * rule1Scale + c * rule2Scale + perceivedVelocity * rule3Scale;
