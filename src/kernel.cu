...
__global__ void kernUpdateVelocityBruteForce(int N, glm::vec3 *pos,
  ...
  // Clamp the speed
  thisVel = glm::clamp(thisVel, -maxSpeed, maxSpeed);
...
__global__ void kernComputeIndices(int N, int gridResolution,
  ...
  glm::vec3 cellIdx = (pos[index] - gridMin) * inverseCellWidth;
...
__global__ void kernUpdateVelNeighborSearchScattered(
...
#if TEST2_2
  glm::ivec3 cellStartIdx = cellIdx - 1.f;
  glm::ivec3 cellEndIdx = cellIdx + 1.f;
#else
  glm::ivec3 cellStartIdx = cellIdx - 0.5f;
  glm::ivec3 cellEndIdx = cellStartIdx + 1;
#endif
  ...
  for (int z = cellStartIdx.z; z <= cellEndIdx.z; z++) {
    if (z < 0 || z >= gridResolution) continue;
    for (int y = cellStartIdx.y; y <= cellEndIdx.y; y++) {
      if (y < 0 || y >= gridResolution) continue;
      for (int x = cellStartIdx.x; x <= cellEndIdx.x; x++) {
        ...
        for (int i = startIndex; i <= endIndex; i++) {
          int indexReal = particleArrayIndices[i];
          if (indexReal == index) continue;
          float dist = glm::distance(pos[indexReal], pos[index]);
          if (dist < rule1Distance) {
            perceivedCenter += pos[indexReal];
            rule1Neighbors++;
          }
          if (dist < rule2Distance) {
            c -= (pos[indexReal] - pos[index]);
          }
          if (dist < rule3Distance) {
            perceivedVelocity += vel1[indexReal];
            rule3Neighbors++;
          }
        }
      }
  ...
  glm::vec3 newVel = vel1[index] + (perceivedCenter - pos[index]) * rule1Scale + c * rule2Scale + perceivedVelocity * rule3Scale;
  newVel = glm::clamp(newVel, -maxSpeed, maxSpeed);
  vel2[index] = newVel;
}

__global__ void kernRearrange(int N, int *indices, glm::vec3 *src, glm::vec3 *dst) {
  int index = (blockIdx.x * blockDim.x) + threadIdx.x;
  if (index < N) {
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  kernRearrange<<<fullBlocksPerGrid2, blockSize>>>(numObjects, dev_particleArrayIndices, dev_pos, dev_pos2);
  kernRearrange<<<fullBlocksPerGrid2, blockSize>>>(numObjects, dev_particleArrayIndices, dev_vel1, dev_vel2);
  // - Perform velocity updates using neighbor search
  kernUpdateVelNeighborSearchScattered<<<fullBlocksPerGrid, blockSize>>>(numObjects, gridSideCount, gridMinimum, gridInverseCellWidth, gridCellWidth, dev_gridCellStartIndices, dev_gridCellEndIndices, dev_particleArrayIndices, dev_pos2, dev_vel2, dev_vel1);
  // - Update positions
